...
import useAddFacturation from '@/utils/hooks/useAddFacturation';


const AddFacturationModal = () => {
    const { calculatedTotalHTVA, modalAddFacturation,
        options, ref, requestCreateProduct, allowSettingProduct
        , setSelectedProduct, setAllowSettingProduct, items, setmodalAddFacturation, appliedTVAInTotal, totalWithTVA
    } = useAddFacturation()
            ...
            <AnimatePresence>
                {
                    ...
                    >
                        <div
                            ...
                            <div className="border-b-[1px] border-black/20 gap-3 flex-wrap">
                                <h2 className="text-[1rem] text-black text-center font-MontBold">Ajout d&apos;une nouvelle facture</h2>
                            </div>
                            <div id='modal-ctx' className="max-[500px]:px-[.5rem] px-[1rem] min-h-[1500px]:max-h-[800px] my-2 py-3">
                                ...
                                {
                                    allowSettingProduct && <>
                                        <p className="font-MontRegular text-[.8rem] text-slate-500 mb-4">Veuillez bien sélectionner un produit</p>
                                        <Select
                                            onChange={(next) => setSelectedProduct([...next as any])}
                                            components={animatedComponents}
                                            closeMenuOnSelect={false}
                                            options={options}
                                            isMulti
                                        />
                                        <div className="text-center text-[.825rem] cursor-pointer active:scale-95 transition-all bg-[var(--variant-primary)] py-1 rounded-full px-4 text-white w-fit mx-auto mt-4 font-MontRegular" onClick={() => {
                                            setAllowSettingProduct(false)
                                        }}>Terminer la sélection de produits</div>
                                    </>
                                }
                                ...
                                {
                                    !allowSettingProduct && <div className="border-t-[1px] border-black/20 pt-5  flex items-center justify-end gap-3 flex-wrap">
                                        <div className="">
                                            <div
